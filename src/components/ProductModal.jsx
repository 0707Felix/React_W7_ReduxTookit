...
const ProductModal = ({
  ...
  useEffect(() => {
    if (isProductModalOpen) {
      ...
      productModal.show();
      productModalRef.current.setAttribute("aria-hidden", "false"); // 顯示時設置為 false
    }
  }, [isProductModalOpen]);
  ...
  const handleCloseProductModal = () => {
    const productModal = Modal.getInstance(productModalRef.current);
    productModal.hide();
    productModalRef.current.setAttribute("aria-hidden", "true"); // 隱藏時設置為 true
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      dispatch(pushMessage({ text: "操作失敗", status: "failed" }));
    }
  };
